...
data_by_medals_country <- data %>%
  group_by(NOC, Year, Medal) %>% 
  summarise(total_medals = n())


data_by_medals_country_avg <- data_by_medals_country %>%
  group_by(NOC, Medal) %>% 
  summarise(avg_medals = ceiling(mean(total_medals)))

data_by_medals_country_avg <- data_by_medals_country_avg %>% 
  rename(
    name = NOC,
    medal = Medal,
  )
...
data_by_medals_country_avg <- merge(
  x=data_by_medals_country_avg,
  y=world_data,
  by='name',
  all.x = T
)

data_by_medals_country_avg[complete.cases(average_medals_by_country),]

max(filter(data_by_medals_country_avg, medal=='Bronze')$avg_medals)


write.csv(data_by_medals_country_avg, '/Users/mrrobot/Downloads/data_by_medals_country_avg.csv', row.names = F)
